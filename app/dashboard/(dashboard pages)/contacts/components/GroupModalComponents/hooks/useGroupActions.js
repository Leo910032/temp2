...
export function useGroupActions(onGroupAction, formState, updateFormState, resetCreateGroupForm) {
    ...
    const handleGenerateAIGroups = useCallback(async (options) => { 
        try {
            console.log("[useGroupActions] Received options to generate:", options);
            
            const result = await onGroupAction('generateAsync', options);
