...
export function useOptimizedEnterpriseData() {
  ...
  return {
    ...
    clearCache: () => cacheService().clearAllCaches(),
...
/**
 * 🚀 REFACTORED: Team-specific data hook
 * Uses the new enhanced services for more granular and robust data fetching.
 */
export function useOptimizedTeamData(teamId) {
  ...
  const fetchTeamData = useCallback(async () => {
    ...
    try {
      ...
      console.log('🚀 REFACTORED: Fetching team data for:', teamId);

      const subService = subscriptionService();
      const team = teamService();
      const invite = invitationService();
      
      // Fetch all data in parallel
      const [userContext, teamDetails, invitations] = await Promise.all([
        subService.getUserContext(),
        team.getTeamDetails(teamId),
        invite.getTeamInvitations(teamId)
      ]);
      ...
      console.log('✅ REFACTORED: Team data loaded:', {
        teamId,
        loadTime: `${loadTime}ms`,
