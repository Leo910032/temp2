...
export class OptimizedPlacesApiClient {
     ...
     * Cost-optimized with minimal field mask
     * Returns: country, city, region, postal code, formatted address
     */
    async reverseGeocode(latitude, longitude) {
        const requestFn = async () => {
            console.log(`🌍 [REVERSE-GEOCODE] Looking up address for: ${latitude}, ${longitude}`);

            // Use the Geocoding API (more accurate and cheaper for reverse geocoding)
            ...
            if (!response.ok) {
                const errorData = await response.json().catch(() => ({}));
                if (response.status === 429) {
                    throw new Error(`Geocoding API quota exceeded: ${errorData.error_message || 'Rate limit hit'}`);
                }
                throw new Error(`Geocoding API failed: ${response.status} - ${errorData.error_message || response.statusText}`);
            }

            const result = await response.json();

            if (result.status !== 'OK' || !result.results || result.results.length === 0) {
                throw new Error(`No address found for coordinates: ${result.status}`);
            }

            // Parse the most relevant result
            const addressData = this.parseGeocodingResult(result.results[0]);

            console.log(`✅ [REVERSE-GEOCODE] Found: ${addressData.city || 'Unknown'}, ${addressData.country || 'Unknown'}`);
    ...
    /**
     * Parse geocoding result into structured address components
     */
    parseGeocodingResult(result) {
        ...
        for (const component of components) {
            ...
            // Street number + route for street address
        ...
        // Construct street address if components available
