...
function buildUnifiedPermissions(subscriptionLevel, teamRoles, organizationRole) {
  ...
  contactConfig.features.forEach(feature => {
   ...
   // ✅ Appearance permissions (This is the block you correctly added)
  ...
  appearanceConfig.features.forEach(feature => {
    ...
    // ✅ ADD THIS NEW BLOCK for Analytics permissions
  const analyticsConfig = ANALYTICS_LIMITS[subscriptionLevel] || ANALYTICS_LIMITS[SUBSCRIPTION_LEVELS.BASE];
  analyticsConfig.features.forEach(feature => { permissions[feature] = true; });
