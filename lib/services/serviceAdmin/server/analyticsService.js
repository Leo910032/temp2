...
export class AnalyticsService {
  ...
  static async _getPlatformApiUsage() {
    try {
      ...
      console.log(`[AnalyticsService] 💰 User IDs from 'users' collection:`, userIds);

      // ALSO check ALL SessionUsage documents to find users with session data
      console.log(`[AnalyticsService] 🔍 Checking SessionUsage collection for any users...`);
      const sessionUsageUsersSnapshot = await adminDb.collection('SessionUsage').listDocuments();
      const sessionUserIds = sessionUsageUsersSnapshot.map(docRef => docRef.id);
      console.log(`[AnalyticsService] 🔍 User IDs in SessionUsage:`, sessionUserIds);

      // ALSO check ALL ApiUsage documents
      const apiUsageUsersSnapshot = await adminDb.collection('ApiUsage').listDocuments();
      const apiUsageUserIds = apiUsageUsersSnapshot.map(docRef => docRef.id);
      console.log(`[AnalyticsService] 🔍 User IDs in ApiUsage:`, apiUsageUserIds);

      // Find users in SessionUsage/ApiUsage but NOT in users collection
      const missingUsers = [
        ...sessionUserIds.filter(id => !userIds.includes(id)),
        ...apiUsageUserIds.filter(id => !userIds.includes(id))
      ];
      if (missingUsers.length > 0) {
        console.warn(`[AnalyticsService] ⚠️ WARNING: Found ${missingUsers.length} users with usage data but NOT in 'users' collection:`, missingUsers);
        console.warn(`[AnalyticsService] ⚠️ These users' data will NOT appear in analytics unless they're added to the users collection`);
      }
      ...
      const sessionUsagePromises = userIds.map(userId =>
        adminDb.collection('SessionUsage')
          ...
          .get()
          .then(snapshot => {
            console.log(`[AnalyticsService] 🔍 DEBUG: User ${userId} has ${snapshot.docs.length} completed sessions`);

            const sessions = snapshot.docs
              .map(doc => {
                const data = doc.data();
                console.log(`[AnalyticsService] 🔍 DEBUG: Session ${doc.id}:`, {
              ...
              .filter(session => {
                ...
                } else if (session.lastUpdatedAt?.toDate && typeof session.lastUpdatedAt.toDate === 'function') {
                  // Fallback to lastUpdatedAt if it's a Firestore Timestamp
                  sessionMonth = session.lastUpdatedAt.toDate().toISOString().slice(0, 7);
                } else if (typeof session.lastUpdatedAt === 'string') {
                  // Fallback to lastUpdatedAt if it's an ISO string
                  sessionMonth = session.lastUpdatedAt.slice(0, 7);
                }

                const isMatch = sessionMonth === currentMonth;
                console.log(`[AnalyticsService] 🔍 DEBUG: Session month=${sessionMonth}, currentMonth=${currentMonth}, match=${isMatch}`);
                return isMatch;
              });

            console.log(`[AnalyticsService] 🔍 DEBUG: After filtering: ${sessions.length} sessions for ${currentMonth}`);
  ...
  static _aggregateUsageData(usageResults, usageType, sessionUsageResults = []) {
    ...
    console.log(`[AnalyticsService] 📋 Looking for usageType: ${usageType} (will match ${usageType}Usage)`);

    let totalSessionsProcessed = 0;
    let totalStepsProcessed = 0;
    let totalStepsMatched = 0;

    sessionUsageResults.forEach(({ userId, sessions }) => {
      if (!sessions || sessions.length === 0) {
        console.log(`[AnalyticsService] 📋 User ${userId}: No sessions to process`);
        return;
      }
      ...
      sessions.forEach((session, sessionIndex) => {
        ...
        if (session.steps && Array.isArray(session.steps)) {
          console.log(`[AnalyticsService] 📋 Session ${sessionIndex} (feature: ${session.feature}): has ${session.steps.length} steps`);
          totalStepsProcessed += session.steps.length;
          ...
          session.steps.forEach((step, stepIndex) => {
            ...
            totalStepsMatched++;
            console.log(`[AnalyticsService] 📋 ✅ Step ${stepIndex}: provider=${step.provider}, feature=${step.feature}, cost=$${step.cost}`);
            ...
            console.log(`[AnalyticsService] 💰 Step ${stepIndex} cost added: $${stepCost.toFixed(4)} → totalCost now $${aggregated.totalCost.toFixed(4)}, totalApiCalls now ${aggregated.totalApiCalls}`);
            ...
            if (!aggregated.featureBreakdown[feature]) {
              ...
              console.log(`[AnalyticsService] 📋 Created new feature breakdown: ${feature}`);
            }
            ...
            console.log(`[AnalyticsService] 📊 Feature "${feature}" now: cost=$${aggregated.featureBreakdown[feature].cost.toFixed(4)}, calls=${aggregated.featureBreakdown[feature].apiCalls}`);
            ...
            if (!aggregated.providerBreakdown[provider]) {
              ...
              console.log(`[AnalyticsService] 📋 Created new provider breakdown: ${provider}`);
            }
            ...
            console.log(`[AnalyticsService] 📊 Provider "${provider}" now: cost=$${aggregated.providerBreakdown[provider].cost.toFixed(4)}, calls=${aggregated.providerBreakdown[provider].apiCalls}`);
          });
        } else {
          console.log(`[AnalyticsService] ⚠️ Session ${sessionIndex}: No steps array found or empty`);
        }
      });
    });

    console.log(`[AnalyticsService] 📋 Session Processing Summary for ${usageType}:`);
    console.log(`[AnalyticsService] 📋   Total sessions: ${totalSessionsProcessed}`);
    console.log(`[AnalyticsService] 📋   Total steps: ${totalStepsProcessed}`);
    console.log(`[AnalyticsService] 📋   Matched steps: ${totalStepsMatched}`);
    console.log(`[AnalyticsService] 📋   Features found: ${Object.keys(aggregated.featureBreakdown).length}`);
    console.log(`[AnalyticsService] 📋   Providers found: ${Object.keys(aggregated.providerBreakdown).length}`);
