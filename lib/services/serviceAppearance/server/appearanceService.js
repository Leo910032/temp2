    ...
    static async updateAppearance({ data, session }) {
        try {
            ...
            if (!session.userId) {
             ...
             // ✅ STEP 2: Use the imported constant for the check.
    ...
    static async uploadFile({ file, uploadType, session }) {
        try {
            ...
            if (!session.userId) {
              ...
              // ✅ STEP 2 (cont.): Use the correct constant here too.
    ...
    static async deleteFile({ deleteType, session }) {
        try {
            ...
            if (!session.userId) {
             ...
             // ✅ It's good practice to have a separate delete permission,
            // but for now, we can reuse the update permission.
            if (!session.permissions[APPEARANCE_FEATURES.CAN_UPDATE_APPEARANCE]) {
