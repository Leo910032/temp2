...
export class AppearanceService {
    ...
    static async updateAppearance({ data, session }) {
        try {
            ...
            const appearanceFields = [
                ...
                // 🆕 Carousel fields
                'carouselEnabled', 'carouselItems', 'carouselStyle',
                // 🆕 CV fields
                'cvEnabled', 'cvItems', 'cvDocument',
                // 🆕 Video Embed fields
                'videoEmbedEnabled', 'videoEmbedItems'
            ];
    ...
    static async uploadFile({ file, uploadType, session }) {
        try {
            ...
            const validUploadTypes = ['profile', 'backgroundImage', 'backgroundVideo', 'bannerImage', 'bannerVideo', 'carouselImage', 'cv'];
    ...
    static _validateField(key, value) {
        ...
        if (key === 'carouselStyle') {
            const validStyles = ['modern', 'minimal', 'bold'];
        ...
        // 🆕 Carousel items validation
        if (key === 'carouselItems') {
            if (!Array.isArray(value)) {
                return { valid: false, error: 'Must be an array' };
            }
            // Validate each item has required structure
            const validatedItems = value.map(item => {
                if (!item.id || !item.title) {
                    throw new Error('Carousel items must have id and title');
                }
                return {
                    id: String(item.id),
            ...
            return { valid: true, sanitizedValue: validatedItems };
        }
