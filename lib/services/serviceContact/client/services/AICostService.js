...
// Client-side service for AI cost tracking and estimation - UPDATED FOR COST TRACKING
...
export class AICostService extends BaseContactService {
  ...
  /**
   * Client-side pricing information for estimates - UPDATED MODEL NAMES
   */
  ...
  /**
   * Get cost estimate for an AI operation - FIXED TO HANDLE BACKEND RESPONSE
   */
  async estimateOperationCost(subscriptionLevel, options = {}) {
    try {
      ...
      const response = await this.cachedRequest(
        ...
        async () => {
          ...
          if (apiResponse && apiResponse.estimate) {
            const estimate = apiResponse.estimate;
            
            // Build features enabled list based on available features and options
            ...
            return {
              ...
              featuresEnabled,
              useDeepAnalysis: estimate.useDeepAnalysis,
              model: estimate.model,
              featuresCount: estimate.featuresCount,
              canAfford: apiResponse.canAfford,
      ...
      return {
        ...
        model: options.useDeepAnalysis ? 'gemini-2.5-pro' : 'gemini-2.0-flash',
        canAfford: false,
        error: error.message
      };
    }
  ...
  /**
   * Client-side cost estimation (for UI updates) - UPDATED MODEL NAMES
   */
  ...
  /**
   * Client-side cost estimation (faster, for UI updates) - UPDATED MODEL NAMES
   */
  getQuickCostEstimate(subscriptionLevel, options = {}) {
    ...
    const modelId = useDeepAnalysis ? 'gemini-2.5-pro' : 'gemini-2.0-flash';
