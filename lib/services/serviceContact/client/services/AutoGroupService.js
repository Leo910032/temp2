...
export class AutoGroupService extends BaseContactService {
  ...
  async generateAutoGroupsAsync(options = {}) {
    console.log("🚀 [Client Service] Starting background AI group generation", { options });

    const result = await ContactApiClient.post('/api/user/contacts/groups/auto-generate-async', 
      { options }, 
      { timeout: 10000 } // Short timeout since we're just starting the job
    );

    console.log("✅ [Client Service] Background job started:", result);
    return result;
  }
