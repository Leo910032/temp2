...
import { ContactServiceFactory } from '../factories/ContactServiceFactory';
...
export class BusinessCardService extends BaseContactService {
   ...
   */
// In ContactService.js, update createContactFromScan method:
...
// ... (imports and class definition) ...
   ...
   * Create contact from scanned business card fields
   */
  async createContactFromScan(userId, scanData) {
    try {
      console.log('📇 Creating contact from scan data for user:', userId, scanData);
      
      const standardFields = scanData.standardFields || [];
      const dynamicFields = scanData.dynamicFields || [];
      ...
      allFields.forEach(field => {
        ...
        // ✅ USE A MORE POWERFUL REGEX FOR SANITIZATION
        // This removes all standard whitespace, non-breaking spaces, and other unicode spaces from the start/end of the string.
        const value = typeof field.value === 'string' 
            ? field.value.replace(/^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g, '') 
            : field.value;

        if (!value) return; // Skip if the value is empty after cleaning

        if (!contactData.name && (label.includes('name') && !label.includes('company'))) {
          contactData.name = value;
        } else if (!contactData.email && label.includes('email')) {
          contactData.email = value;
        } else if (!contactData.phone && label.includes('phone')) {
          contactData.phone = value;
        } else if (!contactData.company && label.includes('company')) {
          contactData.company = value;
        } else if (!contactData.jobTitle && (label.includes('title') || label.includes('job'))) {
          contactData.jobTitle = value;
        } else if (!contactData.website && label.includes('website')) {
          contactData.website = value;
        }
      });

      console.log('📇 Final contact data being sent for creation:', contactData);
      
      // DIAGNOSTIC STEP: Uncomment this to see the character codes if the error persists
      
      if (contactData.name) {
        const charCodes = contactData.name.split('').map(c => c.charCodeAt(0));
        console.log('🕵️‍♂️ Character codes for name:', charCodes);
      }
      
      
      const contactService = ContactServiceFactory.getContactService();
      return await contactService.createContact(userId, contactData);
