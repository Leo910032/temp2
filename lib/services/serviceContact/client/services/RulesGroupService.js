...
export class RulesGroupService {
  // Cache static data as class properties for better performance
   ...
   */
  static estimateProcessingTime(contactCount) {
  ...
  /**
   * Get rules-based feature descriptions (cached)
   */
  static getRulesFeatureDescriptions() {
    return this.FEATURE_DESCRIPTIONS;
  }

  /**
   * Compare rules vs AI features (cached)
   */
  static getFeatureComparison() {
    return this.FEATURE_COMPARISON;
  }

  /**
   * Get rules-based grouping strategies and tips
   */
  static getGroupingStrategies() {
    return {
      companyStrategy: {
   ...
   */
  static validateGroupingOptions(options, subscriptionLevel = 'base') {
    const errors = [];
    const warnings = [];
   ...
   */
  static getRecommendedOptions(contacts = []) {
   ...
   */
  static analyzeContactDataForRecommendations(contacts) {
    const analysis = {
   ...
   */
  static calculateSuccessMetrics(groupingResult, originalContacts) {
