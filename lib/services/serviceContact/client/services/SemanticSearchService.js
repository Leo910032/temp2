...
export class SemanticSearchService {
  ...
  static async rerankContacts(query, contacts, options = {}) {
    const {
      ...
      // REMOVED: model parameter - let server select based on locale
      ...
      detectedLanguage // NEW: UI locale for language-based model selection
    } = options;
  ...
  detectedLanguage: detectedLanguage || 'not detected', // ✅ CORRECT
        modelSelection: 'server-side (based on locale)',
        topN,
      ...
      const rerankResponse = await ContactApiClient.post('/api/user/contacts/rerank', {
        ...
        detectedLanguage // NEW: Pass UI locale for language-based model selection
      });
  ...
  // Extract detected language from vector search response
  const detectedLanguage = vectorSearchResponse.searchMetadata?.detectedLanguage || null;
  
  console.log('[SemanticSearch] Starting reranking with detected language:', detectedLanguage);
  console.log('[SemanticSearch] Rerank will use:', detectedLanguage === 'eng' ? 'English model' : 'Multilingual model');
  ...
  const rerankResponse = await this.rerankContacts(cleanQuery, vectorResults, {
    ...
    trackCosts: true,
    sessionId,
    detectedLanguage // CORRECT - use detected language from query enhancement
  });
