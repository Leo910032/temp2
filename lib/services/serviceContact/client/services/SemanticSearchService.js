...
export class SemanticSearchService {
  ...
  static async rerankContacts(query, contacts, options = {}) {
    const {
      ...
      minConfidence = null, // Minimum rerank relevance threshold
      trackCosts = true,
  ...
  static async search(query, options = {}) {
    ...
    try {
      ...
      if (canRerank && vectorResults.length > 1) {
        console.log('[SemanticSearch] Starting reranking with threshold filtering...');
        const rerankResponse = await this.rerankContacts(cleanQuery, vectorResults, {
          subscriptionLevel,
          topN: maxResults, // Keep for backwards compatibility
          minConfidence: minRerankScore, // Use threshold filtering
          trackCosts: true,
