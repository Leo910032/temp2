// lib/services/serviceContact/client/services/SemanticSearchService.js - ENHANCED WITH CACHING & HISTORY
"use client"
...
export class SemanticSearchService extends BaseContactService {
  ...
  /**
   * Save completed search job to history
   */
  static saveSearchJob(query, results, metadata) {
    try {
      ...
      const job = {
        ...
        // Store actual results with cache key
        cacheKey: `search_job_${jobId}`,
        summary: this.generateJobSummary(results, metadata)
      };

      // Cache the full results separately with longer TTL
      contactCache.set(`search_job_${jobId}`, {
        results,
        metadata,
        query
  ...
  static async loadSearchJob(jobId) {
    try {
      const cachedData = contactCache.cache.get(`search_job_${jobId}`);
      if (cachedData) {
        console.log(`📋 Loaded cached search job: ${jobId}`);
        return cachedData;
      }
      
      console.log(`❌ Search job not found in cache: ${jobId}`);
      return null;
    } catch (error) {
  ...
  /**
   * Enhanced search with smart vector filtering and caching
   */
  static async search(query, options = {}) {
    ...
    try {
      ...
      const cacheKey = `semantic_search_${userId}_${btoa(cleanQuery)}_${subscriptionLevel}_${maxResults}_${enhanceResults}_${streamingMode}`;
  ...
  /**
   * Batch AI enhancement (non-streaming)
   */
  static async enhanceResultsWithAI(query, contacts, options = {}) {
    ...
    try {
      console.log('Starting batch AI enhancement:', {
          ...
          if (aiInsight) {
            return {
              ...
              searchMetadata: {
                ...
                hybridScore: this.calculateHybridScore(contact.vectorScore, aiInsight.confidence)
              }
            };
          }
