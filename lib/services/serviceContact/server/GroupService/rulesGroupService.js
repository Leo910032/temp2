...
export class RulesGroupService {
  ...
  /**
   * FIXED: Save generated groups to Firestore
   * Now properly checks for duplicates BEFORE transaction
   */
  static async saveGeneratedGroups(userId, groups) {
    ...
    try {
      ...
      // FIXED: Get existing groups BEFORE transaction to avoid the contradiction
      ...
      const result = await adminDb.runTransaction(async (transaction) => {
        ...
        const allGroups = [...existingGroups, ...validatedGroups];
        ...
        return {
          success: true,
          savedCount: validatedGroups.length,
