...
export class BusinessCardAI {
     ...
     */
    static async enhanceWithGemini({ extractedText, qrData, imageBase64, side, subscriptionLevel, language, userId, trackCosts = true }) {
        ...
        try {
            ...
            if (trackCosts && userId) {
                try {
                    // Record single operation tracking both cost and usage
                    await CostTrackingService.recordSeparatedUsage(
                        userId,
                        cost,
                        BUSINESS_CARD_AI_CONFIG.MODEL_NAME,
                        'business_card_scan',
                        {
                            side,
                            subscriptionLevel,
                    ...
                    console.log('[AI] 💰 Cost tracked successfully via CostTrackingService');
                } catch (trackingError) {
