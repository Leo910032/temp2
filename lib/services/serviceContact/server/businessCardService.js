  ...
  static async enhanceWithGeminiAI(scanResult) {
    ...
    if (!textToProcess || textToProcess.trim().length < 10) {
      ...
      const qrFields = scanResult.hasQRCode && scanResult.parsedQRData?.contactData 
        ...
        : [];
      return { ...scanResult, extractedFields: qrFields, aiProcessed: false, aiModel: 'none' };
    }
    ...
    try {
      ...
      const prompt = `
        ...
        [...The rest of your excellent prompt stays the same...]
      ...
      // This regex finds a JSON object that might be wrapped in markdown or other text.
