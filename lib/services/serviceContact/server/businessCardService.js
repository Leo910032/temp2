...
export class BusinessCardService {
    ...
    static async processScanRequest({ imageBase64, side, session }) {
        ...
        try {
            ...
            if (actualCost > 0) {
               await recordUsage(session, actualCost, BUSINESS_CARD_AI_CONFIG.MODEL_DISPLAY_NAME, 'business_card_scan', {
    side,
...
static async processBothSides({ frontImage, backImage, session }) {
    ...
    try {
        ...
        // Route based on permissions
        if (session.permissions[CONTACT_FEATURES.AI_ENHANCED_CARD_SCANNER]) {
            console.log(`[BusinessCardService] 🔄 Step 4/5: AI enhancement with combined text...`);
        ...
        // Record usage
        const actualCost = aiResult.cost || 0;
        if (actualCost > 0) {
            await recordUsage(
                session, 
                actualCost, 
                BUSINESS_CARD_AI_CONFIG.MODEL_DISPLAY_NAME, 
                'business_card_scan', 
                {
                    side: 'both',
                    fieldsFound: (finalResult.standardFields?.length || 0) + (finalResult.dynamicFields?.length || 0),
