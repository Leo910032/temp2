...
export class BusinessCardOCR {
    ...
    static async _callGoogleVisionAPI(imageBase64) {
        try {
            ...
            return {
                ...
                confidence: this._calculateConfidenceFromPages(result.fullTextAnnotation),
    ...
    /**
     * Calculate confidence from fullTextAnnotation pages (DOCUMENT_TEXT_DETECTION)
     * This is the correct source for confidence scores in Google Vision API
     */
    static _calculateConfidenceFromPages(fullTextAnnotation) {
        if (!fullTextAnnotation || !fullTextAnnotation.pages || fullTextAnnotation.pages.length === 0) {
            return 0.85; // Default confidence when structure not available
        }

        const confidenceScores = [];

        // Extract confidence from words in pages
        for (const page of fullTextAnnotation.pages) {
            for (const block of page.blocks || []) {
                for (const paragraph of block.paragraphs || []) {
                    for (const word of paragraph.words || []) {
                        if (typeof word.confidence === 'number') {
                            confidenceScores.push(word.confidence);
                        }
                    }
                }
            }
        ...
        if (confidenceScores.length === 0) {
            return 0.85; // Default when no word-level confidence available
        }
    ...
    /**
     * Calculate overall confidence score (DEPRECATED - kept for reference)
     * Note: textAnnotations from TEXT_DETECTION don't include confidence scores
     * Use _calculateConfidenceFromPages() instead
     */
