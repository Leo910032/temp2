...
export class ContactService {
  ...
  static async updateContact(userId, contactId, updates) {
    try {
      console.log('📝 Updating contact:', { userId, contactId });
      ...
      await ContactSecurityService.logContactActivity({
        ...
        contactId,
        details: {
          changes: this.getChangedFields(originalContact, contacts[contactIndex])
        }
      });
   ...
   */
  static async importContacts(userId, contactsData, source = 'import') {
    try {
      ...
      if (contactDoc.exists) {
        existingContacts = contactDoc.data().contacts || [];
      }
        ...
        try {
          ...
          if (contactData.email) {
            const isDuplicate = existingContacts.some(existing => 
              existing.email && existing.email.toLowerCase() === contactData.email.toLowerCase()
            );
  ...
  static async exportContacts(userId, format = 'csv', filters = {}) {
    try {
      ...
      const contactsData = await this.getUserContacts(userId, filters);
  ...
  static async checkContactLimits(userId, operation, count = 1) {
    try {
      ...
      const { getContactLimits } = await import('../constants/contactConstants');
  ...
  static async validateFeatureAccess(userId, feature) {
    try {
      ...
      const { hasContactFeature } = await import('../constants/contactConstants');
  ...
  static async getSubscriptionStatus(userId) {
    try {
      ...
      const { hasContactFeature, getContactLimits, CONTACT_FEATURES } = await import('../constants/contactConstants');
      const limits = getContactLimits(subscriptionLevel);
...
export class ContactGroupService {
  ...
  static async createContactGroup(userId, groupData) {
    try {
      ...
      await ContactService.validateFeatureAccess(userId, 'BASIC_GROUPS');
  ...
  static async generateAutoGroups(userId, options = {}) {
    try {
      ...
      await ContactService.validateFeatureAccess(userId, 'ADVANCED_GROUPS');
