...
import { VectorService } from './vectorService.js'; // Adjust path if necessary
...
export class ContactService {
  ...
  // In createContact method, around line 100, modify the newContact object:
const newContact = {
  ...contactData,
  ...
  dynamicFields: contactData.dynamicFields || []
};
      ...
      await this.createContactVector(userId, newContact);
...
/**
 * Create or update vector for a contact
 * Uses the contact owner's subscription level (not the creator's)
 */
static async createContactVector(userId, contactData) {
  try {
    console.log('🔍 Creating vector for contact:', contactData.id);

    // Get the contact owner's subscription level
    const userDoc = await adminDb.collection('AccountData').doc(userId).get();
    
    if (!userDoc.exists) {
      console.warn('⚠️ User not found for vector creation, skipping');
      return;
    }
    ...
    // Create vector using owner's subscription level
    await VectorService.upsertContactVector(contactWithOwner, subscriptionLevel)
      .catch(error => {
        console.error('❌ Vector creation failed (non-blocking):', error);
        // Don't throw - vector creation failures shouldn't break contact operations
      });
...
code
    ...
    // In updateContact method, after the contact is updated but before the audit log:
// Check if dynamic fields were updated and update vector accordingly
if (updates.dynamicFields !== undefined) {
  await this.updateContactVector(userId, contacts[contactIndex]);
}
...
// In ContactService.js, add or update this method:
static async createContactFromScan(scanData) {
  try {
    console.log('📇 Creating contact from scan data:', scanData);
    ...
    const contactData = {
      ...
      // ✅ ENSURE DYNAMIC FIELDS ARE PRESERVED
      dynamicFields: scanData.dynamicFields || []
    };

    // Map standard fields
    if (scanData.standardFields && Array.isArray(scanData.standardFields)) {
      scanData.standardFields.forEach(field => {
        const label = field.label.toLowerCase();
        if (label.includes('name')) contactData.name = field.value;
        else if (label.includes('email')) contactData.email = field.value;
        else if (label.includes('phone')) contactData.phone = field.value;
        else if (label.includes('company')) contactData.company = field.value;
        else if (label.includes('title')) contactData.jobTitle = field.value;
        else if (label.includes('website')) contactData.website = field.value;
      });
    }

    // ✅ CRITICAL: Call createContact which will trigger vector creation
    return await this.createContact(currentUser?.uid, contactData);
  ...
  /**
 * Update vector for an existing contact
 */
static async updateContactVector(userId, contactData) {
  try {
    console.log('🔄 Updating vector for contact:', contactData.id);
    
    // Get the contact owner's subscription level
    const userDoc = await adminDb.collection('AccountData').doc(userId).get();
    
    if (!userDoc.exists) {
      console.warn('⚠️ User not found for vector update, skipping');
      return;
    }
    ...
    // Update vector using owner's subscription level
   await VectorService.upsertContactVector(contactWithOwner, subscriptionLevel)
        .catch(error => {
          console.error('❌ Vector update failed (non-blocking):', error);
        });
    ...
    try {
      ...
      for (const [index, rawRowData] of contactsData.entries()) {
        ...
        try {
         ...
         // In createContact method, around line 85
const newContact = {
  ...contactData,
  id: `contact_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
  status: contactData.status || CONTACT_STATUS.NEW,
  ...
  static async checkContactLimits(userId, operation, count = 1) {
    try {
      ...
      const { getContactLimits } = await import('../client/constants/contactConstants.js');
...
static getChangedFields(original, updated) {
  ...
  static async validateFeatureAccess(userId, feature) {
    try {
      ...
      const { hasContactFeature } = await import('../client/constants/contactConstants.js');
 ...
 // lib/services/serviceContact/server/contactService.js
  ...
  /**
   * Get subscription status for contact features
   */
  static async getSubscriptionStatus(userId) {
    try {
      console.log('🔍 Getting subscription status for user:', userId);
      
      // Get user document from AccountData collection
      const userDoc = await adminDb.collection('AccountData').doc(userId).get();
      
      if (!userDoc.exists) {
        console.log('❌ User document not found, returning base subscription');
        return {
      ...
      console.log('✅ User subscription level:', subscriptionLevel);
      
      // Get limits and features for this subscription level
      const limits = getContactLimits(subscriptionLevel);
      const canAccessContacts = hasContactFeature(subscriptionLevel, CONTACT_FEATURES.BASIC_CONTACTS);
...
await VectorService.deleteContactVector(contactId)
      .catch(error => {
        console.error('❌ Error deleting contact vector (non-blocking):', error);
        // Don't throw - vector cleanup failure shouldn't break contact deletion
      });
      // Log audit event
      await ContactSecurityService.logContactActivity({
        userId,
