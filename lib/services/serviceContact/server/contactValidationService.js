...
export class ContactValidationService {
  ...
  static validateContactData(contactData) {
    ...
    // Validate individual fields, excluding the old 'phone' field
    Object.keys(CONTACT_VALIDATION).forEach(field => {
      if (field === 'phone') return; // Skip the old single phone validation
    ...
    // NEW: Validate the phoneNumbers array
    if (contactData.phoneNumbers && Array.isArray(contactData.phoneNumbers)) {
      contactData.phoneNumbers.forEach((phoneObj, index) => {
        if (phoneObj && phoneObj.number) {
          const phoneValidation = this.validatePhone(phoneObj.number);
          if (!phoneValidation.isValid) {
            phoneValidation.errors.forEach(err => errors.push(`Phone ${index + 1}: ${err}`));
          }
        }
      });
    }
