...
export class CostTrackingService {
  ...
  static async recordUsage({
    ...
    console.log(`💰 bgtfrdsZ [${operationId}] Recording ${usageType}:`, {
      userId,
      feature,
      cost,
      isBillableRun,
    ...
    try {
      ...
      // **KEY CHANGE**: If a sessionId is provided, record in SessionUsage
      // ✅ FIX: ALSO update user document for billable operations
      if (sessionId) {
        console.log(`📋 [CostTracking] [${operationId}] Session-based operation - recording in SessionUsage`);
        ...
        console.log(`✅ [CostTracking] [${operationId}] Session operation recorded in SessionUsage`);

        // ✅ NEW: For billable operations, ALSO update user document
        // This ensures budget limits and tracking work correctly
        if (isBillableRun || safeCost > 0) {
          console.log(`💰 [CostTracking] [${operationId}] Updating user document for billable session operation`);
          ...
          try {
            await adminDb.runTransaction(async (transaction) => {
              ...
              const userMonthlyUsageMonth = userData.monthlyUsageMonth || null;
              const needsReset = userMonthlyUsageMonth !== currentMonth;

              if (needsReset) {
                console.log(`📅 [CostTracking] [${operationId}] New month detected, resetting user counters (session)`);
            ...
            console.log(`✅ [CostTracking] [${operationId}] User document updated: cost=$${safeCost.toFixed(6)}, billableRun=${isBillableRun}`);
          } catch (error) {
            console.error(`❌ [CostTracking] [${operationId}] Error updating user document for session:`, error);
            // Don't throw - session tracking succeeded, user doc update is supplementary
          }
        }
      ...
      // If NO sessionId, record as a standalone operation in AIUsage/ApiUsage
      console.log(`💾 [CostTracking] [${operationId}] Standalone operation - recording in ${usageType}`);
      ...
      await adminDb.runTransaction(async (transaction) => {
        ...
        // ✅ NEW: Update user document with real-time tracking
        const userData = userDoc.exists ? userDoc.data() : {};
        const userMonthlyUsageMonth = userData.monthlyUsageMonth || null;

        // Check if we need to reset counters for new month
        const needsReset = userMonthlyUsageMonth !== currentMonth;

        if (needsReset) {
          console.log(`📅 [CostTracking] [${operationId}] New month detected, resetting user counters`);
        ...
        console.log(`✅ [CostTracking] [${operationId}] Updated user document with cost: $${safeCost.toFixed(6)}, billableRun: ${isBillableRun}`);

        transaction.set(monthlyDocRef, monthlyData, { merge: true });
        transaction.set(operationDocRef, operationData);
      });

      console.log(`✅ [CostTracking] [${operationId}] Standalone usage recorded:`, {
        usageType,
      ...
      return { success: true, operationId, recordedIn: usageType };
  ...
  static async getUserMonthlyUsage(userId, usageType = 'AIUsage') {
    ...
    try {
      ...
      // ✅ UPDATED: Read from user document (real-time tracking) instead of collections
      // This ensures we're checking the same data that SessionManager uses
      const userDoc = await adminDb.collection('users').doc(userId).get();
      const userData = userDoc.exists ? userDoc.data() : {};
      const userMonth = userData.monthlyUsageMonth || null;

      // If month doesn't match, counters are stale (reset to 0)
      const totalCost = userMonth === currentMonth ? (Number(userData.monthlyTotalCost) || 0) : 0;
      const totalRunsAI = userMonth === currentMonth ? (Number(userData.monthlyBillableRunsAI) || 0) : 0;
      const totalRunsAPI = userMonth === currentMonth ? (Number(userData.monthlyBillableRunsAPI) || 0) : 0;

      // Determine which runs to use based on usageType
      const totalRuns = usageType === 'AIUsage' ? totalRunsAI : totalRunsAPI;

      const monthlyUsage = {
        totalCost,
        totalRuns,
        totalApiCalls: 0, // Not tracked in user document, only in collections
        featureBreakdown: {},
        providerBreakdown: {},
