...
export class CostTrackingService {
  ...
  static async recordUsage({
    ...
    try {
      ...
      // **DUAL-WRITE ARCHITECTURE**: If a sessionId is provided, write to BOTH:
      // 1. SessionUsage (detailed step tracking for auditing)
      // 2. Monthly aggregation docs (fast dashboard queries)
      //
      // This approach provides:
      // - Fast analytics queries (read pre-aggregated monthly docs)
      // - Detailed audit trails (session steps for debugging)
      // - Eliminates need for expensive session aggregation at query time
      if (sessionId) {
        console.log(`📋 [CostTracking] [${operationId}] Session-based operation - DUAL-WRITE to SessionUsage + monthly docs`);
        ...
        // Also update monthly aggregation document for fast dashboard queries
        // (Code continues below to avoid duplication)
        console.log(`✅ [CostTracking] [${operationId}] Session step recorded in SessionUsage, now updating monthly docs...`);
        // Fall through to monthly doc update logic below
      }

      // Update monthly aggregation documents (for BOTH session and standalone operations)
      const operationType = sessionId ? 'Session-based' : 'Standalone';
      ...
      console.log(`✅ [CostTracking] [${operationId}] Usage recorded:`, {
        ...
        isBillableRun,
        sessionId: sessionId || 'none',
        recordedIn: sessionId ? 'SessionUsage + monthly docs' : usageType
      });
