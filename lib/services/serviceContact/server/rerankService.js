...
// ENHANCED: Dynamic model selection based on query language detection (English vs Multilingual)
 ...
 * RerankService
 * 
 * ENHANCED WITH QUERY ENHANCEMENT INTEGRATION:
 * - Accepts detectedLanguage from QueryEnhancementService
 * - Dynamically selects rerank model: English vs Multilingual
 * - Fallback to multilingual if language not detected
 * - Comprehensive logging of model selection reasoning
 */
export class RerankService {
  static async rerankContacts(query, contacts, options = {}) {
    const {
      ...
      detectedLanguage = null // NEW: Accept detected language from query enhancement
    } = options;

    console.log(`🔄 [RerankService] [${rerankId}] Starting rerank:`, {
      query: query.substring(0, 50) + '...',
      contactsCount: contacts.length,
      detectedLanguage: detectedLanguage || 'not provided',
      topN
    });
    ...
    try {
      ...
      // This is the key integration with QueryEnhancementService
      // ========================================
      let model;
      let modelSelectionReason;

      if (detectedLanguage) {
        // Language was detected by QueryEnhancementService
        if (detectedLanguage === 'eng') {
          model = 'rerank-english-v3.0';
          modelSelectionReason = 'english_detected_by_ai';
          console.log(`🌐 [RerankService] [${rerankId}] ✅ Using ENGLISH model:`, {
            detectedLanguage,
            model,
            reason: 'Query enhancement detected English language'
          });
        } else {
          model = 'rerank-multilingual-v3.0';
          modelSelectionReason = 'non_english_detected_by_ai';
          console.log(`🌐 [RerankService] [${rerankId}] ✅ Using MULTILINGUAL model:`, {
            detectedLanguage,
            model,
            reason: `Query enhancement detected ${detectedLanguage} language`
          });
        }
      } else {
        // No language detected - use multilingual as safe fallback
        model = 'rerank-multilingual-v3.0';
        modelSelectionReason = 'no_language_detected_fallback';
        console.log(`🌐 [RerankService] [${rerankId}] ⚠️ Using MULTILINGUAL model (FALLBACK):`, {
          detectedLanguage: 'none',
          model,
          reason: 'No language detected - using multilingual as safe fallback'
        });
      }

      console.log(`📊 [RerankService] [${rerankId}] MODEL SELECTION SUMMARY:`, {
        finalModel: model,
        detectedLanguage: detectedLanguage || 'unknown',
        selectionReason: modelSelectionReason,
        isOptimal: !!detectedLanguage
      });
      ...
      const rerankedContacts = filteredResults.map((result, rank) => {
        ...
        return {
          ...
          searchMetadata: {
            ...
            rerankModel: model,
            detectedLanguage: detectedLanguage || 'unknown', // NEW: Include detected language
            modelSelectionReason, // NEW: Include model selection reasoning
      ...
      const result = {
        ...
        metadata: {
          cost: actualCost,
          model,
          detectedLanguage: detectedLanguage || 'unknown', // NEW
          modelSelectionReason, // NEW
          modelOptimal: !!detectedLanguage, // NEW: Flag if model selection was based on AI detection
