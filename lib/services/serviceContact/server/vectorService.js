...
static async getIndex() {
  ...
  try {
    ...
    if (this._indexInstance) {
      AdvancedLogger.debug('Pinecone', 'index_cache_hit', {
        duration: Date.now() - indexStartTime
      });
      return this._indexInstance;
    }

    AdvancedLogger.info('Pinecone', 'index_initialization_start', {
    ...
    AdvancedLogger.debug('Pinecone', 'index_list_retrieved', {
    ...
    if (!indexExists) {
      AdvancedLogger.info('Pinecone', 'index_creation_start', {
        indexName: INDEX_NAME,
        dimension: 768
      });
      ...
      AdvancedLogger.info('Pinecone', 'index_creation_initiated', {
    ...
    AdvancedLogger.info('Pinecone', 'index_ready', {
      duration: Date.now() - indexStartTime,
      indexName: INDEX_NAME
    });
    ...
    AdvancedLogger.error('Pinecone', 'index_initialization_failed', {
      duration: Date.now() - indexStartTime,
      error: error.message,
      indexName: INDEX_NAME
    });
...
static async getEmbedding(text) {
  ...
  try {
    ...
    AdvancedLogger.debug('Gemini', 'embedding_request_start', {
    ...
    AdvancedLogger.info('Gemini', 'embedding_generated', {
      duration: embeddingTime,
    ...
    AdvancedLogger.error('Gemini', 'embedding_generation_failed', {
...
static buildContactDocument(contact, subscriptionLevel) {
  ...
  AdvancedLogger.debug('VectorService', 'document_build_start', {
  ...
  if (subscriptionLevel === 'premium') {
    ...
    AdvancedLogger.debug('VectorService', 'document_build_complete', {
      tier: 'premium',
  ...
  if (isBusinessOrHigher) {
    AdvancedLogger.debug('VectorService', 'business_tier_enhancement', {
      hasNotes: !!contact.notes,
      hasMessage: !!contact.message,
      hasDetails: !!contact.details,
      hasLocation: !!contact.location?.address
    });
  ...
  AdvancedLogger.debug('VectorService', 'document_build_complete', {
    ...
    documentPreview: finalDocument.substring(0, 200) + (finalDocument.length > 200 ? '...' : ''),
    contactId: contact.id
  });
...
static async upsertContactVector(contact, ownerSubscriptionLevel) {
  ...
  try {
    ...
    try {
      ...
      AdvancedLogger.info('VectorService', 'vector_upsert_success', {
        contactId: contact.id,
        contactName: contact.name,
        namespace,
        embeddingDimension: embedding.length,
    ...
    AdvancedLogger.error('VectorService', 'vector_upsert_failed', {
      contactId: contact?.id,
      contactName: contact?.name,
      error: error.message,
      totalDuration: Date.now() - totalStartTime
    ...
    if (error.message?.includes('not ready') || error.message?.includes('initializing')) {
      AdvancedLogger.info('VectorService', 'vector_queued', {
        contactId: contact?.id,
        message: 'Vector will be processed once index is ready'
      });
      return;
    }
  ...
  static async search(query, userId, subscriptionLevel) {
    ...
    try {
      ...
      AdvancedLogger.info('VectorService', 'search_complete', {
        userId,
        resultsCount: orderedResults.length,
        subscriptionLevel,
        queryLength: query.length
      ...
      AdvancedLogger.error('VectorService', 'search_failed', {
        userId,
        subscriptionLevel,
        error: error.message,
  ...
  static async getContactsByIds(userId, contactIds) {
    ...
    try {
      ...
      AdvancedLogger.debug('VectorService', 'contacts_retrieved', {
        userId,
        totalContacts: allContacts.length,
        matchedContacts: matchedContacts.length,
      ...
      AdvancedLogger.error('VectorService', 'contacts_fetch_failed', {
        userId,
        error: error.message,
        contactIdsCount: contactIds.length
      });
  ...
  static async deleteContactVector(contactId, userId) {
    ...
    try {
      ...
      AdvancedLogger.info('VectorService', 'vector_deleted', {
        contactId,
        userId,
        namespace
      });
      ...
      AdvancedLogger.error('VectorService', 'vector_delete_failed', {
        contactId,
        userId,
        error: error.message
      });
  ...
  static async rebuildUserVectors(userId, newSubscriptionLevel) {
    ...
    try {
        ...
        await Promise.all(
          batch.map(async (contact) => {
            try {
              ...
              errors++;
              AdvancedLogger.error('VectorService', 'rebuild_contact_failed', {
                contactId: contact.id,
                contactName: contact.name,
                error: error.message
              });
      ...
      AdvancedLogger.info('VectorService', 'rebuild_complete', {
        userId,
        newSubscriptionLevel,
        totalContacts: contacts.length,
        rebuilt,
      ...
      AdvancedLogger.error('VectorService', 'rebuild_user_vectors_failed', {
        userId,
        newSubscriptionLevel,
        error: error.message,
  ...
  /**
   * Get index statistics - UPDATED WITH LOGGING
   */
  static async getIndexStats() {
    ...
    try {
      ...
      AdvancedLogger.info('VectorService', 'index_stats_retrieved', {
        vectorCount: result.vectorCount,
        indexFullness: result.indexFullness,
        dimension: result.dimension,
        namespacesCount: Object.keys(result.namespaces || {}).length,
        duration: Date.now() - startTime
      });
      ...
      AdvancedLogger.error('VectorService', 'index_stats_failed', {
        error: error.message,
        duration: Date.now() - startTime
      });
  ...
  static async cleanupOrphanedVectors(userId) {
    ...
    try {
      ...
      if (orphanedIds.length > 0) {
        ...
        for (let i = 0; i < orphanedIds.length; i += batchSize) {
          ...
          try {
            ...
            AdvancedLogger.error('VectorService', 'orphan_batch_delete_failed', {
              batchIds: batch,
              error: error.message
            });
          }
        ...
        AdvancedLogger.info('VectorService', 'orphaned_vectors_cleaned', {
          userId,
          deleted,
          found: orphanedIds.length,
          duration: Date.now() - startTime
        ...
        AdvancedLogger.info('VectorService', 'no_orphaned_vectors', {
          userId,
          duration: Date.now() - startTime
        });
      ...
      AdvancedLogger.error('VectorService', 'cleanup_orphaned_vectors_failed', {
        userId,
        error: error.message,
        duration: Date.now() - startTime
      });
  ...
  static async testVectorOperations(userId) {
    ...
    try {
      ...
      AdvancedLogger.info('VectorService', 'test_operations_complete', {
        userId,
        embeddingDimensions: testEmbedding.length,
        searchResultsCount: searchResults.length,
        indexStatsAvailable: !!stats,
      ...
      AdvancedLogger.error('VectorService', 'test_operations_failed', {
        userId,
        error: error.message,
        duration: Date.now() - startTime
      });
  ...
  static async migrateToGeminiEmbeddings(userId) {
    ...
    try {
      ...
      AdvancedLogger.info('VectorService', 'migration_to_gemini_complete', {
        userId,
        subscriptionLevel,
        vectorsRebuilt: result.rebuilt,
      ...
      AdvancedLogger.error('VectorService', 'migration_to_gemini_failed', {
        userId,
        error: error.message,
        duration: Date.now() - startTime
      });
  ...
  static async batchMigrateToGemini(batchSize = 10) {
    ...
    try {
        ...
        await Promise.all(
          batch.map(async (userId) => {
            try {
              ...
              AdvancedLogger.debug('VectorService', 'user_migrated', {
                userId,
                batchNumber
              });
              ...
              AdvancedLogger.error('VectorService', 'user_migration_failed', {
                userId,
                batchNumber,
                error: error.message
              });
      ...
      AdvancedLogger.info('VectorService', 'batch_migration_complete', {
        totalUsers: userIds.length,
        migrated,
        errors,
        successRate: ((migrated / userIds.length) * 100).toFixed(2) + '%',
        totalDuration: Date.now() - startTime
      });
      ...
      AdvancedLogger.error('VectorService', 'batch_migration_failed', {
        error: error.message,
        duration: Date.now() - startTime
      });
