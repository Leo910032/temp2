 ...
 */
// Replace the getImpersonatedAnalytics function in your optimizedEnterpriseService.js with this:

export async function getImpersonatedAnalytics(targetUserId, teamId, period = '30d') {
    const startTime = Date.now();
    
    try {
        console.log(`🔍 Impersonating analytics for user: ${targetUserId} in team: ${teamId}`);

        // ✅ Create cache key for impersonated analytics
        const cacheKey = `impersonated_analytics_${targetUserId}_${teamId}_${period}`;
        
        // ✅ Try to get from cache first (with shorter TTL for analytics)
        return await cache.getOrFetch(
            cacheKey,
            async () => {
                // ✅ Get authentication headers (reuse existing pattern)
                const headers = await getAuthHeaders();
                
                // ✅ Construct API URL
                const url = `/api/user/analytics/impersonate/${targetUserId}?teamId=${teamId}&period=${period}`;
                ...
                if (!response.ok) {
                    ...
                    if (response.status === 403) {
                        throw new Error('You do not have permission to view this user\'s analytics');
                    } else if (response.status === 404) {
                        throw new Error('User not found or not in your team');
                    } else if (response.status === 400) {
                        throw new Error('Invalid request parameters');
                    } else {
                        throw new Error(errorData.error || 'Failed to fetch impersonated analytics');
                    }
                }

                const data = await response.json();
                
                // ✅ FIXED: Return the complete data structure exactly as received from the API
                // The backend is already processing and returning the correct structure
                return {
                    success: true,
                    impersonationContext: data.impersonationContext,
                    analytics: {
                        // ✅ FIXED: Include ALL the view properties that are missing
                        totalViews: data.totalViews,
                        todayViews: data.todayViews,
                        yesterdayViews: data.yesterdayViews,
                        thisWeekViews: data.thisWeekViews,
                        thisMonthViews: data.thisMonthViews,
                        
                        // Keep existing click properties
                        totalClicks: data.totalClicks,
                        todayClicks: data.todayClicks,
                        yesterdayClicks: data.yesterdayClicks,
                        thisWeekClicks: data.thisWeekClicks,
                        thisMonthClicks: data.thisMonthClicks,
                        ...
                        trafficSourceStats: data.trafficSourceStats,
                        username: data.username,
                        
                        // Include any other properties from the API response
                        period: data.period,
                        days: data.days,
                        avgClicksPerDay: data.avgClicksPerDay,
                        topCountries: data.topCountries,
                        analyticsDataCount: data.analyticsDataCount,
        ...
        console.error(`❌ Impersonated analytics failed after ${duration}ms:`, error);
        
        // ✅ Update error stats
        updateCacheStats('impersonated_analytics_error', duration, false);
        ...
        // ✅ Always update stats
        updateCacheStats('impersonated_analytics', duration, true);
        
        console.log(`✅ Impersonated analytics completed in ${duration}ms`);
    }
}
 ...
 */
export async function canImpersonateAnalytics(targetUserId, teamId) {
    try {
        console.log(`🔒 Checking impersonation permissions for user: ${targetUserId}`);

        const cacheKey = `can_impersonate_${targetUserId}_${teamId}`;
        ...
        return await cache.getOrFetch(
            ...
            async () => {
                ...
                // ✅ We can create a lightweight permissions check endpoint
                const response = await fetch(`/api/enterprise/teams/${teamId}/members/${targetUserId}/can-impersonate`, {
                    method: 'GET',
                    headers
                });
 ...
 */
export async function getImpersonationAuditLog(teamId, limit = 50) {
    try {
        console.log(`📋 Fetching impersonation audit log for team: ${teamId}`);

        const cacheKey = `audit_log_${teamId}_${limit}`;
        ...
        return await cache.getOrFetch(
            ...
            async () => {
                ...
                const response = await fetch(`/api/enterprise/teams/${teamId}/audit-logs?action=ANALYTICS_IMPERSONATION&limit=${limit}`, {
                    method: 'GET',
                    headers
                });
                ...
                if (!response.ok) {
                    const errorData = await response.json();
                    throw new Error(errorData.error || 'Failed to fetch impersonation audit log');
                }
            ...
            'permissions' // Short cache since audit logs should be fresh
        );
...
/**
 * 🚀 OPTIMIZED: Get the current user's own pending invitations for the banner
 */
export async function getPendingUserInvitations() {
  const userId = auth.currentUser?.uid;
  if (!userId) return []; // Return empty array if not logged in
  
  // A unique cache key for the current user's personal invites
  const cacheKey = `user_pending_invites_${userId}`;
  
  return await cache.getOrFetch(
    cacheKey,
    async () => {
      const headers = await getAuthHeaders();
      // ✅ Using the correct API route for this purpose
      ...
      if (!response.ok) {
        // Don't throw an error that would break the UI, just return empty
        console.warn('Could not fetch pending user invitations.');
        return [];
      }

      return await response.json();
    },
    'invitations' // Use the standard 'invitations' cache TTL
  );
}
/**
...
/**
 * 🚀 OPTIMIZED: Accept an invitation with smart cache invalidation
 */
export async function acceptInvitation(invitationId) {
  try {
    console.log('✅ Accepting invitation via service:', invitationId);
    ...
    // ✅ THIS IS THE CRITICAL FIX FOR THE STALE CACHE
    if (result.teamId) {
      console.log(`🧹 Invalidating caches for team ${result.teamId} after a member joined.`);
      // Invalidate the cache for team members
      invalidateRelatedCaches('member_operation', { teamId: result.teamId });
      // Invalidate the cache for team invitations
      invalidateRelatedCaches('invitation_operation', { teamId: result.teamId });
    }
    
    console.log('✅ Invitation accepted successfully by service');
    return result;

  } catch (error) {
    console.error('❌ Error accepting invitation in service:', error);
    throw error;
  }
}
// ==================== BULK OPERATIONS ====================
