...
export class EnhancedSubscriptionService extends BaseService {
  ...
  // lib/services/serviceEnterprise/client/services/EnhancedSubscriptionService.js
// Add these methods to the existing EnhancedSubscriptionService class

// ==================== MISSING METHODS FROM OPTIMIZED SERVICE ====================

/**
 * Get user context with enhanced caching (moved from optimizedEnterpriseService.js)
 */
async getUserContext() {
  return this.cachedRequest(
    'user_context',
...
/**
 * Enhanced batch data fetch (moved from optimizedEnterpriseService.js)
 */
async getEnterpriseDataBatch() {
  return this.cachedRequest(
    'enterprise_batch',
    async () => {
      console.log('🚀 Executing BATCH enterprise data fetch via Enhanced Service');
      
      // Use other Enhanced Services for better separation
      const { ServiceFactory } = await import('../factories/ServiceFactory');
      
      // Parallel fetch using Enhanced Services
      const [subscriptionStatus, userContext, userTeams] = await Promise.allSettled([
        this.getEnterpriseSubscriptionStatus(),
        this.getUserContext(),
      ...
      console.log('✅ BATCH enterprise data completed via Enhanced Service:', {
        hasSubscription: !!result.subscriptionStatus,
        hasUserContext: !!result.userContext,
        teamsCount: result.userTeams.teams?.length || 0,
        errorCount: result.errors.length
...
/**
 * Preload enterprise data for better performance
 */
async preloadEnterpriseData() {
  try {
    console.log('⚡ Preloading enterprise data via Enhanced Service...');
    ...
    if (userId) {
      ...
      Promise.allSettled([
        this.getEnterpriseDataBatch(),
        this.getUserContext(),
        this.getEnterpriseSubscriptionStatus()
      ]).catch(console.warn);
    }
    
    console.log('⚡ Enhanced Service preload initiated');
    return { success: true };
  } catch (error) {
    console.warn('⚠️ Enhanced Service preload failed:', error);
