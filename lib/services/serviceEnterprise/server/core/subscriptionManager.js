...
export class SubscriptionManager {
  ...
  static async getComprehensiveStatus(userId) {
    try {
      ...
      // Now, call all helper methods PASSING IN the userData to avoid re-fetching.
      ...
      const operationPermissions = await this._getOperationPermissionsFromData(userId, userData, subscriptionStatus);
   ...
   * 🚀 NO DATABASE CALLS IN LOOP.
   */
  static async _getOperationPermissionsFromData(userId, userData, status) {
    ...
    for (const operation of operations) {
      // Pass userData to avoid refetching inside validateEnterpriseOperation
      const result = await this._validateEnterpriseOperationFromData(userData, operation, {});
   ...
   * 🚀 NO DATABASE CALL HERE.
   */
  static async _validateEnterpriseOperationFromData(userData, operation, context = {}) {
      const subscriptionLevel = userData.accountType?.toLowerCase() || 'free';
      const userRole = this.getUserRole(userData);
      const config = this.getSubscriptionConfig(subscriptionLevel);

      if (!this.hasEnterpriseAccessForLevel(subscriptionLevel)) {
        return { /* ... enterprise required response ... */ };
      }
      
      // Operation-specific validations (these are fast, no DB calls)
      return await this.validateSpecificOperation(
