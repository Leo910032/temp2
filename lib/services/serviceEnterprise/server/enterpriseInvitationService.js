...
// Fixed import - use direct import instead of class import
import { logAuditEvent } from './enterpriseSecurityService';
// lib/services/serviceEnterprise/server/enterpriseInvitationService.js

import { DEFAULT_PERMISSIONS_BY_ROLE, INVITATION_STATUS } from '@/lib/services/constants'; // ✅ CHANGED to the central barrel fileimport crypto from 'crypto';
...
export class EnterpriseInvitationService {
    ...
    static async createInvitation(inviterId, organizationId, teamId, invitedEmail, role) {
        ...
        // Check for existing invitations and handle them gracefully
        ...
        if (!existingInviteQuery.empty) {
            ...
            if (existingData.status === INVITATION_STATUS.EXPIRED) {
                ...
                // Fixed: Use direct function call instead of class method
                await logAuditEvent({
                    userId: inviterId,
                    organizationId,
    ...
    static async resendInvitation(invitationId, resenderId = null) {
        ...
        // Fixed: Use direct function call
        await logAuditEvent({
    ...
    static async _sendInvitationEmail(inviterId, organizationId, teamId, invitedEmail, inviteCode, type = 'new') {
        try {
            ...
            if (inviterDoc.exists && orgDoc.exists) {
                ...
                console.warn('Missing inviter or organization data:', {
