...
export class EnterpriseInvitationService {
    ...
    resourceType: 'team',      // ✅ Correct: The resource is the team
    resourceId: teamId,        // ✅ Correct: The ID of the resource is the team's ID
    details: { 
        invitedEmail, 
        role,
        invitationId: docRef.id // Add the specific invitation ID to the details
    } 
});
        ...
        return await adminDb.runTransaction(async (transaction) => {
            ...
            try {
                await EnterpriseSecurityService.logAuditEvent({
                    ...
                    details: { teamId: result.teamId, role: invitation.role }
                });
            } catch (auditError) {
